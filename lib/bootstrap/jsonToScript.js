...
const normalizeContentTypeID = (id) => id.replaceAll('-', '_')
...
const createField = (itemId, field) => `
  ${normalizeContentTypeID(itemId)}.createField("${field.id}")${''.concat(...Object.entries(field.props).filter(rejectEmptyObjects).map(createProp))};
`

const createChangeFieldControl = (itemId, field) => {
  const { fieldId, widgetId, settings } = field
  const baseString = `${normalizeContentTypeID(itemId)}.changeFieldControl("${fieldId}", "builtin", "${widgetId}"`
  return settings ? `${baseString}, ${JSON.stringify(settings)});` : `${baseString});`
}

const createContentType = (item, editorInterface) => `
  const ${normalizeContentTypeID(item.id)} = migration.createContentType('${item.id}')${''.concat(...Object.entries(item.props).map(createProp))};
